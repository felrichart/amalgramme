...
@keyframes drop {
  0% { transform: translateY(-1.6rem) rotate(-10deg); opacity: 0; }
  60% { transform: translateY(0.15rem) rotate(3deg); opacity: 1; }
...
.sub { margin: 0.25rem 0 1.3rem; color: var(--muted); font-weight: 600; }
.time {
  display: flex;
...
.time .label { font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.12em; color: var(--muted); }
.time .value { font-size: 1.8rem; font-weight: 800; color: var(--sky-ink); }
.cta {
  border: 1px solid var(--glass-brd);
...
.cta:hover { box-shadow: 0 12px 26px rgba(70, 100, 150, 0.24); }
.cta:active { transform: scale(0.96); }
.cta:focus-visible { outline: 2px solid var(--sky-ink); outline-offset: 2px; }
