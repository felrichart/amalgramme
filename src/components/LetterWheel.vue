...
const glyph = computed(() => (nodeSize.value * (props.interactive ? 0.32 : 0.42)).toFixed(2) + 'cqw')
...
<template>
  ...
  >
    ...
    <div v-if="interactive" class="hub" aria-hidden="true">
      <svg viewBox="0 0 24 24"><path fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" d="M4 12a8 8 0 0 1 13.7-5.6L20 8M20 3v5h-5M20 12a8 8 0 0 1-13.7 5.6L4 16M4 21v-5h5"/></svg>
    </div>
  </div>
</template>
...
.wheel.shaking { animation: shake 0.4s ease-in-out; }

.links { position: absolute; inset: 0; width: 100%; height: 100%; z-index: 1; overflow: visible; }
...
.node span { display: block; line-height: 1; }
.node.on {
...
.hub svg { width: 46%; height: 46%; }

.mini .node {
  background: linear-gradient(160deg, rgba(255, 255, 255, 0.8), rgba(255, 255, 255, 0.4));
  box-shadow: inset 0 1px 1px rgba(255, 255, 255, 0.8), 0 2px 6px rgba(60, 80, 120, 0.1);
}

/* A soft damped wobble — enough to read as "no", never jarring. */
@keyframes shake {
  15% { transform: translateX(-1.5%); }
  35% { transform: translateX(1.2%); }
  55% { transform: translateX(-0.8%); }
  75% { transform: translateX(0.4%); }
}
@media (prefers-reduced-motion: reduce) {
  .wheel.shaking { animation: none; }
}
</style>
